...
contract Charityable is Context, Ownable {
        ...
        onlyOwner
    {
        require(
            _charity == address(0),
            "Charity address cannot be changed once set"
        );
...
contract SafeKomodo is Context, IBEP20, Charityable {
    ...
    string private _name = "Safe Komodo";
    ...
    uint256 private _charityPercentageOfLiquidity = 25;
    ...
    constructor(address _routerAddress) public {
        ...
        //exclude owner, charity address and this contract from fee
    ...
    function _transferToCharity(uint256 amount) private {
        _totalCharityCollected = _totalCharityCollected.add(amount);
        charity().transfer(amount);
        emit CharityCollected(_totalCharityCollected);
    }
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length(); i++) {
            if (
                _rOwned[_excluded.at(i)] > rSupply ||
                _tOwned[_excluded.at(i)] > tSupply
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 balanceToCharity =
            newBalance.mul(charityPercentageOfLiquidity().div(10**2));
        uint256 tokensExtra =
            otherHalf.mul(charityPercentageOfLiquidity().div(10**2));

        //transfer balanceToCharity
        _transferToCharity(balanceToCharity);
